...
In this chapter, we presented the P2DE, a linear estimator of delay domain CSI which uses the frequency domain pilots as specified by 4G and 5G protocols. We demonstrated the effectiveness of this estimator as an input to a range of deep CNNs, including autoencoders and deep compressive sensing networks. We also expanded on our work in differential encoding, proposing a heterogeneous differential encoder which combines autoencoders and deep compressive sensing networks to improve the network's estimation accuracy.

While this chapter highlighted the effectiveness of deep compressive sensing networks for CSI compression, such networks can be computationally expensive. In the following chapter, we will explore some methods for reducing the computational complexity of these networks.