  ...
  \subsubsection{Convolutional Neural Networks}
    ...
    \begin{frame}{Convolutional Neural Networks (CNNs)}
      \begin{itemize}
        ...
        \pause
        \item Instantaneous decoding (non-iterative). % No assumptions on sparsity/RIP.
      \end{itemize}
...
% \subsection{Spherical Normalization}
    ...
    \begin{table}[htb]
      \begin{center}
        ...
        \begin{tabular}{|l|c|c|}
          ...
          Num. BS Antennas ($N_b$) & \multicolumn{2}{c|}{32} \\ \hline
  ...
  \begin{frame}{Experimental Results}
    \begin{figure}[!hbtp] \centering 
      ...
      \caption{Sensitivity study for CsiNet-Pro and spherical normalization \cite{ref:liu2020sphnet} (lower NMSE is better).}
...
\section{Completed Work \#3: Pilots-to-delay Estimator (P2DE) and Heterogeneous Differential Encoding}
  ...
  % \begin{frame}{Downsampled Frequency Domain CSI}
  %   $\boldeta_{d,i}$ related to $\boldeta_i$ by pilot matrix, $\mathbf{P}_i$, as
  %   \begin{equation}
  %     \boldeta_{d,i} = \boldeta_i \mathbf{P}_i \; \forall \; i \in [1, \dots, N_b],
  %   \end{equation}
  %   for $i$-th antenna port.

  %   $\mathbf{P}_i =$ matrix of one-hot vectors -- conforms with 3GPP-defined pilot allocation.
  % \end{frame}

  \nofoot{
  \begin{frame}{Downsampled Frequency Domain CSI}
    \begin{itemize}
      \item $M_f = $ \# downsampled subcarriers 
      \item $\text{DR}_f = \frac{M_f}{N_f}$ (frequency downsampling ratio)
    \end{itemize}
    \begin{figure}[!hbtp]
    \centering
    {
  ...
  \begin{figure}[!hbtp]
    \centering
    {
      \fontsize{8pt}{8pt}
      \def\svgwidth{1.0\columnwidth}
      \input{../images/p2de_process.pdf_tex}
    }
    \label{fig:p2de_process}
  \end{figure}
  \end{frame}
  ...
  \begin{frame}{P2DE Results: Frequency Downsampling Rate}
    Accuracy of the P2DE at the UE (i.e., before compression and feedback) for different $\text{DR}_f$.
    \begin{figure}[!hbtp]
        \centering
  ...
  \footnotesize{
  \begin{itemize}
    \item {\color{red}\textbf{MN-ISTANet+}}: Better performance in first timeslot.
    \item {\color{darkgreen}\textbf{MN-ENet}}: Better performance in error timeslots.
    \item {\color{blue}\textbf{MN-IE}}: Enjoys both of the above benefits.
  \end{itemize}
  \vspace{-2mm}
  \begin{figure}[!hbtp]
    \centering
    \includegraphics[width=\linewidth]{outdoor_markov_sweep_v2.pdf}
    \caption{Differential encoding networks with linear P2DE as input ($M_f=128, \text{DR}_f=\frac{1}{8}, D=4$).}
...
\section*{Appendix}
  ...
  \begin{frame}{Off-diagonal Regularization}
    For a nearly singular matrix, stability of the matrix inverse can be improved by regularization. We use off-diagonal regularization (ODIR) to perform such regularization.

    Denote $A_{ij}$ as the $i$-th row and $j$-th column of matrix $\mathbf{A}$. Select a scaling factor $\delta \in \mathbb{R}^{+}$ to scale down off-diagonal elements of $\mathbf{A}$. The elements of the resulting ODIR matrix, $\mathbf{A}_{\text{ODIR}}$, are written as
    \begin{align}
        A_{ij, \text{ODIR}} &= 
            \begin{cases}
                A_{ij} & \text{if } i = j\\
