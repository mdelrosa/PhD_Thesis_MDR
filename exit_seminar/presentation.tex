...
\section{Current Work: Pilot Feedback and Model Re-use}
  ...
  \begin{frame}{Model Re-use}
    ...
    which compresses CSI at the UE by matrix multiplication, 
    \begin{align}
        f(x) &= \mathbf{\Phi}\mathbf{x}
    \end{align}
    for the measurement matrix $\mathbf{\Phi}$ and 
    $\mathbf{x}$ is the flattened CSI matrix of $K$ contiguous subcarriers across
    all angular indices, i.e.,
    \begin{itemize}
      \item $\mathbf{x} \in \mathbb{R}^{N_K}$
      \item $\mathbf{\Phi} \in \mathbb{R}^{\text{CR} K  \times K}$ 
    \end{itemize}
    where $N_K = 2 K N_b$ is the dimension of
    the $K$ subcarriers from the CSI matrix.
  \end{frame}

  \nofoot{
  \begin{frame}{Direct+Re-use Results}
    \footnotesize{
      \begin{itemize}
  ...
  \nofoot{
  \begin{frame}{Pilot Feedback + Model Reuse: Conclusions}
    \footnotesize{
    \begin{itemize}
